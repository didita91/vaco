 ...
 */
public class VoneProblemUsandoRSA extends Problem {
    ...
    public VoneProblemUsandoRSA(List<NodosFisicosCandidatos> lista, SustrateNetwork redFisic, VirtualNetwork redVirt) throws IOException, CloneNotSupportedException {
        listaNodos = lista;
        redFisica = redFisic;
        redVirtual = redVirt;
                    ...
                    for (VirtualEdge ev : redVirtual.getEnlacesVirtuales()) {
                        if ((ev.getNodoUno().getIdentificador() == nodoOrigen && ev.getNodoDos().getIdentificador() == nodoDestino)
                            ...
                            try {
                                ...
                                Logger.getLogger(VoneProblemUsandoRSA.class.getName()).log(Level.SEVERE, null, ex);
                            }
    ...
    public String toString() {
        return VoneProblemUsandoRSA.class.getSimpleName();
    }
...
//        }
    ...
    private double obtenerDistancia(int i, int j) throws CloneNotSupportedException {
        ...
        double dist = obtenerDistanciaRSA(origen, destino, origenv, destinov);
        int fs = obtenerFS(origenv, destinov);
    ...
    private void obtenerCaminosRSA() throws CloneNotSupportedException {

        for (NodosFisicosCandidatos lsO : listaNodos) {
            for (NodosFisicosCandidatos lsD : listaNodos) {

                if (lsO.getNodoFisico().getIdentificador() != lsD.getNodoFisico().getIdentificador()) {
                    if (distance[lsO.getNodoFisico().getIdentificador()][lsD.getNodoFisico().getIdentificador()] == 0.0
                            && distance[lsO.getNodoFisico().getIdentificador()][lsD.getNodoFisico().getIdentificador()] != 9999999
                            && distance[lsD.getNodoFisico().getIdentificador()][lsO.getNodoFisico().getIdentificador()] != 9999999) {

                        double dist = obtenerDistanciaRSA(lsO.getNodoFisico().getIdentificador(), lsD.getNodoFisico().getIdentificador(), lsO.getNodoVirtual().getIdentificador(), lsD.getNodoVirtual().getIdentificador());
                        distance[lsO.getNodoFisico().getIdentificador()][lsD.getNodoFisico().getIdentificador()] = dist;
                        distance[lsD.getNodoFisico().getIdentificador()][lsO.getNodoFisico().getIdentificador()] = dist;
                    }
                }
            }
        }
