...
public class Dijkstra {
	...
	public static ShortestPathTree shortestPathTree(Graph graph,
		...
		DijkstraNode sourceNode = predecessorTree.getNodes().get(
				predecessorTree.getRoot());
		...
		while (!pq.isEmpty()) {
			...
			visited.add(current);
			count++;
			HashMap<String, Double> neighbors = nodes.get(currLabel)
					.getNeighbors();
			for (String currNeighborLabel : neighbors.keySet()) {
				DijkstraNode neighborNode = predecessorTree.getNodes().get(
						currNeighborLabel);
				Double currDistance = neighborNode.getDist();
				Double newDistance = current.getDist()
						+ nodes.get(currLabel).getNeighbors()
								.get(currNeighborLabel);
				if (newDistance < currDistance) {
					DijkstraNode neighbor = predecessorTree.getNodes().get(
							currNeighborLabel);
	...
	public static Path shortestPath(Graph graph, String sourceLabel,
		...
		while (!pq.isEmpty()) {
			...
			if (currLabel.equals(targetLabel)) {
				...
				while (parentN != null) {
					shortestPath.addFirst(new Edge(parentN, currentN, nodes
							.get(parentN).getNeighbors().get(currentN)));
					currentN = parentN;
