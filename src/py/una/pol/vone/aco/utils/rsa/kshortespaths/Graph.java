	...
	public Graph transpose() {
		...
		while (it.hasNext()) {
			...
			while (alIt.hasNext()) {
				...
				newNodes.get(neighborLabel).addEdge(nodeLabel,
						neighbors.get(neighborLabel));
			}
		}
		...
		try {
			...
			while (line != null) {
				...
				if (edgeDescription.length == 3) {
					addEdge(edgeDescription[0], edgeDescription[1],
				...
				if (nodes.containsKey(nodeLabel)) {
					...
					while (it2.hasNext()) {
						...
						if (nodes.containsKey(nodeLabel2)) {
							writer.write(nodeLabel + " " + nodeLabel2 + " "
									+ neighbors.get(nodeLabel2) + "\n");
						}
					}
				}
			}
