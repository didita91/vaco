...
public class Path implements Cloneable, Comparable<Path> {
	 ...
	 * Path path = (Path) o;
	 * 
	 * if (Double.compare(path.totalCost, totalCost) != 0) return false; if
	 * (!edges.equals(path.edges)) return false;
	 * 
	 * return true; }
	 * 
	 * @Override public int hashCode() { int result; long temp; result =
	 * edges.hashCode(); temp = Double.doubleToLongBits(totalCost); result = 31
