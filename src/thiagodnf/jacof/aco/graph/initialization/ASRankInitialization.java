...
/**
 * Pheromone initialization for Rank-based Ant System (ASRank) algorithm. This
 * value is based on Travelling Salesman Problem and was extracted from:
 * <br><br>
 * Bullnheimer, Bernd, Richard F. Hartl, and Christine Strauss. "A new rank
 * based version of the Ant System. A computational study." (1997).
 *
...
//		checkArgument(weight >= 0, "The weight should be greater or equal than 0.0");
        if (rate < 0.0) {
            System.err.println("The rate should be greater or equal than 0.0");
        }
        if (rate > 1.0) {
            System.err.println("The rate should be less or equal than 1.0");
        }
        if (weight < 0) {
            System.err.println("The weight should be greater or equal than 0.0");
        }
        this.rate = rate;
        this.weight = weight;
    }
