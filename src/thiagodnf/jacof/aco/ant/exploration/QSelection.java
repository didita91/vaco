...
//        checkArgument(q0 <= 1.0, "The q0 should be less or equal than 1.0");
        if (q0 < 0.0) {
            System.err.println("The q0 should be greater or equal than 0.0");
        }
        if (q0 > 1.0) {
            System.err.println("The q0 should be less or equal than 1.0");
        }
        this.q0 = q0;
    }
    ...
    protected int doExploitation(Ant ant, int i) {
        ...
        for (Integer j : ant.getNodesToVisit()) {
            ...
            if (aco.getGraph().getTau(i, j) == 0.0) {
                System.err.println("The tau(i,j) should not be 0.0");
            }
            double tij = aco.getGraph().getTau(i, j);
            double nij = Math.pow(aco.getProblem().getNij(i, j,ant.tour), aco.getBeta());
        ...
        if (nextNode == -1) {
            System.err.println("The next node should not be -1");
        }
        return nextNode;
    }
