...
/**
 * This class represents an ant and its process of building a solution for a
 * given addressed problem.
 *
...
public class Ant extends Observable implements Runnable {
    ...
    public Ant(ACO aco, int id) {
        ...
        if (aco == null) {
            System.err.println("The aco should not be null");
        }
        if (id < 0) {
            System.err.println("The id should be > 0. Passed: %s" + id);
    ...
    public void reset() {
        ...
        if(this.tour.isEmpty()){
           this.currentNode=aco.getListaPosicionInicial().get(0);
        }
        this.currentNode = aco.getListaPosicionInicial().get(id % aco.getListaPosicionInicial().size());
        this.tourLength = 0.0;
        this.tour.clear();
...
//
//            writer.write("Ant_" + id + " " + tourLength + " " + tour);
//            writer.newLine();
//
//            writer.flush();
